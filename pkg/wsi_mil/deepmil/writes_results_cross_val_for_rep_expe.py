...
def select_best_repeat(df, sgn_metric, ref_metric, path, rep, n_best=1):
    ...
    for t in tests:
        ...
        assert rep <= len(metric_t), "rep too big"
        ids = np.random.choice(range(len(metric_t)), (rep), replace=False)
        best_indices =  ids[np.array(np.argsort(metric_t[ids])[:n_best])]
        best_rep = df.loc[best_indices, 'repeat']
        best_rep = [int(x) for x in best_rep]
        selection.append(zip([int(t)] * n_best, best_rep))
    return selection

def copy_best_to_root(inpath, param):
    ...
    for p in param:
        t, r = p
        model_path = os.path.join(inpath, "test_{}/rep_{}/model_best.pt.tar".format(t, r))
...
def main(raw_args=None):
    ...
    models_params = select_best_repeat(df=df, sgn_metric=sgn_metric, ref_metric=ref_metric, path=args.path, rep=args.rep, n_best=args.n_ensemble)
